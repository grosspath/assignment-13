...
var reverseObject = function (thingObject) {
 var newObject = {};

for (var prop in thingObject) {

  if (thingObject.hasOwnProperty(prop) === true) {

    newObject[thingObject[prop]] = prop;
  }
}
  return newObject;
}
var reverseEverything = function (someObject) {

  var newArray = [];

  for ( var i = someObject.length - 1; i >= 0; i--) {

   var arrayItem = someObject[i];
   newArray.push(reverseObject(arrayItem));
